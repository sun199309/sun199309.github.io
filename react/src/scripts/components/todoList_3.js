...
export default class TodoList2 extends Component{
    ...
    componentWillMount(){
        axios.get("http://localhost:1902/react/commonts")
        .then(res=>{
            console.log(res);
            this.setState({
...
class ShowMsgCom extends Component{
    ...
    del(id,_id){
        ...
        var item = "item"+id;
        axios.get("http://localhost:1901/react/delcommont",{
            params:{
                _id
            }
        }).then(
...
class PostMsgCom  extends Component{
    change=()=>{
        ...
        this.refs.content.state.value = "";
        addComment(title,content);
		axios.get("http://localhost:1902/react/addcommont",{params:{title,content}})
		.then(res=>{
		    console.log(res);
		    this.setState({
		        comments:res.data.result
